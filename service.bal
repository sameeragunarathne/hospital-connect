...
service /fhir/r4 on new http:Listener(servicePort) {
    ...
    resource function post MedicationRequest(@http:Payload json payload) returns json {
        r4:OperationOutcome operationOutcome = {issue: [{severity: "information", code: "informational", diagnostics: "Not implemented"}]};
        return operationOutcome.toJson();
    }

    // Read medication request by ID
    resource function get MedicationRequest/[string id]() returns json|error {
        ...
        r4:Resource|error fhirResourcesFromCcda = getFhirResourcesFromCcda(ccdaResponse, "MedicationRequest", patient, id);
        if fhirResourcesFromCcda is r4:Resource {
            return fhirResourcesFromCcda.toJson();
        }
        r4:OperationOutcome operationOutcome = {issue: [{severity: "error", code: "exception", diagnostics: "Failed to construct FHIR medicationrequest resources"}]};
        return operationOutcome.toJson();
    }

    // Search medication requests
    resource function get MedicationRequest(string? patient) returns json|error {
        if patient is () {
            r4:OperationOutcome operationOutcome = {issue: [{severity: "error", code: "processing", diagnostics: "Patient reference is required"}]};
            return operationOutcome.toJson();
        }
        ...
        r4:Resource|error fhirResourcesFromCcda = getFhirResourcesFromCcda(ccdaResponse, "MedicationRequest", patient);
        if fhirResourcesFromCcda is r4:Bundle {
            return fhirResourcesFromCcda.toJson();
        }
        r4:OperationOutcome operationOutcome = {issue: [{severity: "error", code: "exception", diagnostics: "Failed to construct FHIR medicationrequest resources"}]};
    ...
    // Read specific version of medication request
    resource function get MedicationRequest/[string id]/_history/[string vid]() returns json {
        r4:OperationOutcome operationOutcome = {issue: [{severity: "information", code: "informational", diagnostics: "Not implemented"}]};
        return operationOutcome.toJson();
    }

    // Delete medication request
    resource function delete MedicationRequest/[string id]() returns json {
        r4:OperationOutcome operationOutcome = {issue: [{severity: "information", code: "informational", diagnostics: "Not implemented"}]};
        return operationOutcome.toJson();
    }
}